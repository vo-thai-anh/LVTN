...
const StatusBadge = ({ status }) => {
  ...
  // Nếu trạng thái không khớp, hiển thị chính giá trị đó hoặc nhãn mặc định
  ...
  return (
    <span className="ds-badge" style={{ color: config.color, backgroundColor: config.bg }}>
...
const OrderManagement = () => {
  ...
  const fetchOrders = async () => {
    try {
      ...
      setOrders(data || []);
      // console.log('>>> [DEBUG] Cấu trúc dữ liệu API trả về:', data);
      // if (Array.isArray(data) && data.length > 0) {
      //   console.log('>>> [DEBUG] Keys của đối tượng đơn hàng đầu tiên:', Object.keys(data[0]));
      //   console.log('>>> [DEBUG] Ví dụ giá trị một đơn hàng:', data[0]);
      // }
    } catch (err) {
      console.log('Lỗi tải danh sách đơn hàng',err);
    } finally {
      setLoading(false);
    }
  };
  ...
  const filteredOrders = Array.isArray(orders) ? orders.filter(o => {
    const orderId = o?.don_hang_id?.toString() || '';
    const customerName = o?.khach_hang?.ten_khach_hang?.toLowerCase() || '';
            ...
            {loading ? (
              ...
              filteredOrders.map((order) => (
                ...
                <tr key={order?.don_hang_id}>
                  <td style={{ fontSize: '13px', color: 'var(--admin-text-light)', fontWeight: 500 }}>#{order?.don_hang_id}</td>
                  <td>
                    <div style={{ fontWeight: 500, color: 'var(--admin-text-head)', fontSize: '14px' }}>{order?.khach_hang?.ten_khach_hang}</div>
                    <div style={{ fontSize: '12px', color: 'var(--admin-text-muted)' }}>{order?.sdt_nguoi_nhan}</div>
                  </td>
                  <td style={{ fontSize: '13px' }}>
                    {order?.ngay_tao ? new Date(order.ngay_tao).toLocaleDateString('vi-VN') : '—'}
                  </td>
                  <td>
                    <div style={{ fontWeight: 500, color: 'var(--admin-primary)' }}>{order?.tong_tien?.toLocaleString()}đ</div>
                  </td>
                  <td className="text-center">{order?.so_luong_sach || 0}</td>
                  ...
                  <td className="text-center">
                    ...
                    <button
                      onClick={() => handleUpdateStatus(order.don_hang_id, 'ĐANG_GIAO_HÀNG')}
                      className="ds-btn ds-btn-success"
                      style={{ marginLeft: '5px' }}>
                      Xác nhận
                    </button>
                    )}
                    {order?.trang_thai === 'ĐANG_GIAO_HÀNG' && (
                      <button onClick={() => handleUpdateStatus(order.don_hang_id, 'ĐÃ_GIAO_HÀNG')}
                    ...
                    {order?.trang_thai == 'CHỜ_XÁC_NHẬN' && (
                    <button
                      onClick={() => handleUpdateStatus(order.don_hang_id, 'ĐÃ_HỦY')}
            ...
            >
              <div style={{ padding: '20px 32px', borderBottom: '1px solid var(--admin-divider)', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                <div>
                  ...
                  <div style={{ fontSize: '12px', color: 'var(--admin-text-light)', marginTop: '4px' }}>Đặt ngày {new Date(selectedOrder.ngay_tao).toLocaleString('vi-VN')}</div>
                </div>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '24px' }}>
                  <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                       ...
                       <div>
                          ...
                          <p style={{ margin: 0, fontSize: '14px', fontWeight: 500 }}>{selectedOrder.khach_hang?.ten_khach_hang}</p>
                       </div>
                    </div>
                    <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                       ...
                       <div>
                          ...
                          <p style={{ margin: 0, fontSize: '14px', fontWeight: 500 }}>{selectedOrder.sdt_nguoi_nhan}</p>
                       </div>
                    </div>
                  </div>
                  <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
                  ...
                  <div style={{ gridColumn: 'span 2', background: 'var(--admin-bg-ash)', padding: '16px', borderRadius: '4px', marginTop: '8px' }}>
                     <div style={{ display: 'flex', alignItems: 'flex-start', gap: '10px' }}>
                        ...
                        <div>
                          ...
                          <p style={{ margin: '4px 0 0', fontSize: '14px', lineHeight: 1.5 }}>{selectedOrder.dia_chi_giao_hang}</p>
                        </div>
                     </div>
                  </div>
                  ...
                  {selectedOrder.ghiChu && (
                    <div style={{ gridColumn: 'span 2', padding: '12px 16px', borderLeft: '3px solid var(--admin-divider)' }}>
                      ...
                      <p style={{ margin: '4px 0 0', fontSize: '13px', fontStyle: 'italic' }}>"{selectedOrder.ghi_chu}"</p>
                    </div>
                  )}
                  ...
                  <div style={{ gridColumn: 'span 2', borderTop: '1px solid var(--admin-divider)', paddingTop: '24px', marginTop: '16px' }}>
                     <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '8px' }}>
                        ...
                        <span style={{ fontWeight: 500 }}>{selectedOrder.so_luong_sach} cuốn</span>
                     </div>
                     <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', fontSize: '18px', marginTop: '16px' }}>
                        ...
                        <span style={{ fontWeight: 600, color: 'var(--admin-primary)' }}>{selectedOrder.tong_tien?.toLocaleString()}đ</span>
                     </div>
                  </div>
                </div>
