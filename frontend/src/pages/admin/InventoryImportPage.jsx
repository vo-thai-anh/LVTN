...
import { 
  ...
  Search, Eye, X, Loader2, History, ClipboardList
} from 'lucide-react';
    ...
    return (
        <div className="inventory-container animate-in">
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '24px', flexWrap: 'wrap', gap: '16px' }}>
                <div>
                    <h2 className="inventory-header-title">Quản Lý Nhập Kho</h2>
                    <p className="inventory-header-subtitle">
                        Nhập sách mới vào kho và quản lý danh sách lô hàng đã nhập.
                    </p>
                </div>
            ...
            {activeTab === 'history' ? (
                <div className="inventory-card">
                    <div className="inventory-card-title-bar">
                        <span className="inventory-card-title-text">DANH SÁCH LỊCH SỬ NHẬP KHO</span>
                    ...
                    <div className="inventory-table-container">
                        {loadingHistory ? (
                            <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', padding: '60px 0', gap: '12px' }}>
                                ...
                                <span style={{ color: 'var(--inv-text-muted)', fontSize: '13px' }}>Đang tải lịch sử nhập kho...</span>
                            </div>
                        ) : history.length === 0 ? (
                            <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', padding: '60px 0', gap: '8px' }}>
                                ...
                                <span style={{ color: 'var(--inv-text-dark)', fontSize: '14px', fontWeight: 600 }}>Chưa có phiếu nhập kho nào</span>
                                <button 
                                    onClick={() => setActiveTab('create')} 
                                            ...
                                            <tr key={phieu.phieu_nhap_id}>
                                                ...
                                                <td style={{ color: 'var(--inv-text-muted)' }}>
                                                    <span className="inventory-badge inventory-badge-blue">
                                                        <ClipboardList size={13} /> {totalItems} sản phẩm
                                                    </span>
                                                </td>
                                                <td style={{ fontWeight: 700, color: 'var(--inv-success)' }}>
                ...
                <form onSubmit={handleSubmit} className="inventory-card animate-in">
                    <div className="inventory-card-title-bar">
                        <span className="inventory-card-title-text">Lập Phiếu Nhập Kho Mới</span>
                    </div>

                    <div className="inventory-form-row">
                        <div className="inventory-input-group">
                            <label className="inventory-label">
                                <Calendar size={15} color="var(--inv-primary)" /> Ngày Nhập Kho
                            </label>
                            <input 
                                type="date" 
                        ...
                        <div className="inventory-input-group">
                            <label className="inventory-label">
                                <FileText size={15} color="var(--inv-primary)" /> Ghi Chú / Nhà Cung Cấp
                            </label>
                            <input 
                                type="text" 
                                className="inventory-input" 
                                placeholder="Ví dụ: Nhập kho nhà sách Kim Đồng, Người giao: Nguyễn Văn A..."
                                value={ghiChu}
                    ...
                    <div className="inventory-items-section">
                        <h4 className="inventory-section-title">
                            <ClipboardList size={16} color="var(--inv-primary)" /> Danh Sách Sách Nhập Kho
                        </h4>
                                                        ...
                                                        filteredBooks.map(book => (
                                                            ...
                                                            >
                                                                ...
                                                                <div className="inventory-book-info">
                                                                    ...
                                                                    <div className="inventory-book-stock">Hiện tại tồn: {book.soLuong} cuốn</div>
                                                                </div>
                                                            </div>
                                                        ))
                                                    )}
                        ...
                        <div className="inventory-summary-footer">
                            <button 
                                ...
                                className="inventory-btn inventory-btn-secondary" 
                            >
                                <Plus size={16} /> Thêm sách cần nhập
                            </button>

                            <div style={{ display: 'flex', alignItems: 'center', gap: '24px', flexWrap: 'wrap', justifyContent: 'flex-end' }}>
                                <div style={{ textAlign: 'right' }}>
                                    <span className="inventory-grand-total-label">TỔNG TIỀN PHIẾU:</span>
                ...
                <div className="inventory-modal-overlay" onClick={() => setShowDetailModal(false)}>
                    <div className="inventory-modal-card animate-in" onClick={(e) => e.stopPropagation()}>
                        ...
                        <div className="inventory-modal-body">
                            <div className="inventory-modal-info-box">
                                <div className="inventory-modal-info-item">
                                    <span className="inventory-modal-info-label">Ngày Nhập Kho</span>
                                    <span className="inventory-modal-info-value">
                                        {new Date(selectedPhieu.ngay_nhap).toLocaleDateString('vi-VN')}
                                    </span>
                                </div>
                                <div className="inventory-modal-info-item">
                                    <span className="inventory-modal-info-label">Tổng Tiền Lô Hàng</span>
                                    <span className="inventory-modal-info-value" style={{ color: 'var(--inv-success)' }}>
                                        {formatCurrency(selectedPhieu.tong_tien)}
                                    </span>
                                </div>
                                <div className="inventory-modal-info-item" style={{ gridColumn: 'span 2' }}>
                                    <span className="inventory-modal-info-label">Ghi Chú / Nguồn hàng</span>
                            ...
                            <div className="inventory-modal-items-list">
                                {selectedPhieu.chi_tiet?.map((item, index) => {
                                    ...
                                    return (
                                        <div key={item.chi_tiet_phieu_nhap_id || index} className="inventory-modal-item-row">
                                            {sach.anh_bia ? (
